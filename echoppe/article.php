...
class ArticleController extends Controller {
    ...
    public function afficherArticle() {
        ...
        if (!isset($_GET['i_idRayon'])) {
            ...
            $max_article = 2;
            $num_article_cat = array();
            foreach ($to_descriptionArticle as $o_descriptionArticle) {
                $num_article_cat[$o_descriptionArticle['id_categorie']] ++;
            }

            if (($num_article / $max_article) % 1 != 0)
                $page_supp = 1;
            else
                $page_supp = 0;
            ...
            if ($num_article > $max_article) {
                if (!isset($_GET['i_pageNum'])) {
                    ...
                    foreach ($to_categorie as $o_categorie) {
                        if (($i_pageNum - 1) * $max_article <= $num_article_cat[$o_categorie['id']] + $nb_article_pre) {
                            if ($num_article_cat[$o_categorie['id']] + $nb_article_pre < ($i_pageNum) * $max_article) {
                                $fin_pre = ($i_pageNum - 1) * $max_article - $num_article_cat[$o_categorie['id']] % $max_article; // dernier numéro d'article de la page précédente
                            } else{
                                $fin_pre = ($i_pageNum - 1) * $max_article; // dernier numéro d'article de la page précédente
                            echo $fin_pre;
                            }
                        }
...
//            }
    ...
    public function afficherCreerArticle() {
        ...
        $to_categorie = Categorie::getAllObjects();
        $this->render('creerArticle', compact('o_rayon', 'to_tva', 'to_unite', 'to_fournisseur', 'to_categorie', 'i_erreur'));
    }
